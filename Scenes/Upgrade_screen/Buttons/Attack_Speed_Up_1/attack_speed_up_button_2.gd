...
func _process(delta):
	if ButtonGlobals.second_attack_speed_upgrade_useable:
		_attack_up()
	
func _attack_up():
	var _is_transparent: bool = false
	var _current_upgrade_amount_count = ButtonGlobals.second_attack_speed_upgrade_count
	...
	if !_is_transparent:
		...
		if Input.is_action_just_pressed("mouse_click") and _upgrade:
			...
			ButtonGlobals.second_attack_speed_upgrade_count += 1
			GameState.score -= _upgrade_cost
			anim.play("press")
		if _current_upgrade_amount_count == _max_upgrade:
			_is_transparent = true
			ButtonGlobals.third_attack_speed_upgrade_useable = true
			ButtonGlobals.second_attack_speed_upgrade_useable = false
	if _is_transparent:
