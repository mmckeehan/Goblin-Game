...
func _process(delta):
	if ButtonGlobals.third_game_time_upgrade_useable:
		_attack_up()
	
func _attack_up():
	var _is_transparent: bool = false
	var _current_upgrade_amount_count = ButtonGlobals.third_game_time_upgrade_count
	...
	if !_is_transparent:
		...
		if _current_upgrade_amount_count == _max_upgrade:
			...
			ButtonGlobals.third_game_time_upgrade_useable = false
	if _is_transparent:
