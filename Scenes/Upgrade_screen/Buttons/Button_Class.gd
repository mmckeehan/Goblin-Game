...
func _attack_up():
	var _current_upgrade_amount_count = ButtonGlobals.first_rock_value_upgrade_count #Will need to figure out how to override per the script
	...
	if _current_upgrade_amount_count == _max_upgrade:
		_sprite.set_self_modulate("ffffff8d")
